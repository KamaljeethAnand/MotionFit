...
# from tkeras.models import model_from_json
import numpy as np
import random
...
class FacialExpressionModel(object):
    ...
    def __init__(self, model_json_file, model_weights_file):
        ...
        #     self.loaded_model = tf.compat.v1.keras.models.model_from_json(loaded_model_json)
    ...
    def predict_emotion(self, img):
        ...
        return np.argmax(random.choice([0,1,2,3,4,5,6]))
import cv2
import numpy as np
...
def init():
    ...
    obj=page(window)
    typ=2
    obj.addgame("Temp Run 2 ","Louise_Patra/Temple_Run_2/",typ=typ)
    obj.addgame("Hill Climb ","Louise_Patra/Hill_Climbing/",typ=typ)
    obj.addgame("Angry Birds ","Louise_Patra/Angry_Birds/",typ=typ)
    ...
    obj.addgame("Subway Surfers ","Kamaljeet/Subway_Surfers/",typ=2)
    ...
    def run(self):
        ...
        while True:
            ...
            for (x, y, w, h) in faces:
                ...
                roi = cv2.resize(fc, (48, 48))
                # pred = model.predict_emotion(roi[np.newaxis, :, :, np.newaxis])
                pred=random.choice(["Angry", "Disgust",
                    "Fear", "Happy",
                    "Neutral", "Sad",
                    "Surprise"])
                l.append(pred)
